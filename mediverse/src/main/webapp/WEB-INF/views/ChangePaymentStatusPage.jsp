...
<head>
    ...
    <title>Mediverse Admin - Change Payment Status</title>
    <style>
        /* Reusing styles from OrderManagementPage.jsp */
                    ...
                    <tbody>
                        <c:forEach var="payment" items="${payments}">
                            <tr>
                                ...
                                <td><strong>${payment.paymentStatus}</strong></td>
                                <td>
                                    <form action="/admin/updatePaymentStatus" method="POST" style="display: flex; gap: 10px;">
                                        ...
                                        <select name="newStatus" required>
                                            ...
                                            <c:forEach var="status" items="${statuses}">
                                                <option value="${status}" 
                                                    <c:if test="${status eq payment.paymentStatus}">selected</c:if>>
                                                    ${status}
                                                </option>
                                            </c:forEach>
                                        </select>
